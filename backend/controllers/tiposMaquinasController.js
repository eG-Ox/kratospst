...
exports.eliminarTipoMaquina = async (req, res) => {
  try {
    ...
    res.json({ mensaje: 'Tipo de máquina eliminado exitosamente' });
  } catch (error) {
    console.error('Error eliminando tipo de máquina:', error);
    if (error.code === 'ER_ROW_IS_REFERENCED_BY_CONSTRAINT') {
      return res.status(400).json({ error: 'No se puede eliminar: hay máquinas usando este tipo' });
    }
